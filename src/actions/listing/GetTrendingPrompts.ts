...
export default async function GetTrendingPrompts(network: TNetwork, n: number = 10, type?: TPromptType) {
    await connectDB();

    const query: any = [{ $match: { network } }, { $sort: { date: -1, sales: 1 } }, { $limit: n }];
